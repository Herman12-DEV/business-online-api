...
export class ReportsService {
  ...
  // Graphique — évolution du bénéfice par période
  const chartData = this.buildChartData(currentSales, period);
  ...
  if (currentSalesAgg._count > 0) {
    if (profitTrend !== null && Math.abs(profitTrend) > 1) {
      if (profitTrend > 0) {
        insight = `Ton bénéfice progresse de ${profitTrend.toFixed(1)} % par rapport à la période précédente.`;
      } else {
        insight = `Ton bénéfice est en baisse de ${Math.abs(profitTrend).toFixed(1)} % par rapport à la période précédente.`;
      }
    } else if (margin > 30) {
